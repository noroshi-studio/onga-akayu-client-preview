...
通常は同一タブのセッション中に一度だけ、約3.6秒表示されます。再確認するときは、公開URLまたはローカルURLの末尾へ `?intro=1` を付けます。
...
営業時間、料金、お知らせ、臨時休業・特別営業、特別料金の注意文は、将来のWordPress化時に管理画面から編集できる項目として移行します。今回は静的プレビューのみを更新しています。

## 正式ロゴと地図

施設提供の正式な透過PNGを `static-preview/assets/images/logo/` に格納しています。`Logo_1.png` はオープニングとヒーロー、`Logo_2.png` はヘッダーとフッター、`Logo_3.png` はファビコンに使用します。ロゴは変形・トリミング・再着色せず、`object-fit: contain` で表示します。

アクセス欄のGoogleマップはAPIキー不要のiframeで住所を表示し、施設から提供されたGoogleマップURLを外部リンクに使用しています。将来のWordPress版でもロゴ画像と地図URLを引き継ぎます。
